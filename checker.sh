...
WORKSPACE=/Users/joey/Downloads/mining_scripts/isOnline
...
function test {
  ...
  if [ $response -eq 200 ] ; then
    ...
    # website down
    if [ "$QUIET" = false ] ; then 
	echo -n "$response "; echo -e "\e[31m[DOWN]\e[0m"; 
	# should shutdown reboot the server 
	/sbin/shutdown -r +$TIMETOREBOOT; 
    fi
    # comment out cause if dont want to send emails
